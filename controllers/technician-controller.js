...
const getTechnician = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({ message: `Failed to get technician with ID: ${id}` });
  }
};
...
const createTechnician = async (req, res) => {
  ...
  try {
    ...
    console.log(error);
    res.status(500).json({ error: `Failed to add new technician item ${error}` });
  }
};
...
const updateTechnician = async (req, res) => {
    ...
    const { technicianName, technicianEmail, technicianPhone } = req.body;
  try {
    const updatedTechnician = await technician.findByIdAndUpdate(id, 
        { technicianName, technicianEmail, technicianPhone } , { new: true });
    ...
    console.error(error);
    res.status(500).json({ message: `Failed to update technician info with ID: ${id}` });
  }
};
...
const deleteTechnician = async (req, res) => {
  ...
  try {
    ...
    if (!deletedTechnician) {
      return res.status(404).json({ message: `Technician with ID: ${id} not found` });
    }
    res.status(200).json(deletedTechnician);
  } catch (error) {
    res.status(500).json({  message: `Failed to delete technician with ID: ${id}` });
  }
};
